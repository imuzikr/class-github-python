(function () {
  ...
  function explainPythonError(message) {
    ...
    if (message.includes("TypeError")) {
      return "서로 맞지 않는 자료형을 함께 사용했을 가능성이 큽니다. 예를 들어 숫자와 문자열을 +로 연결하려면 숫자를 str()로 바꾸어야 합니다.";
    }
    ...
    return "오류 메시지의 마지막 줄을 먼저 읽어 보세요. 어떤 줄에서 문제가 생겼는지 확인한 뒤 변수 이름, 따옴표, 괄호, 자료형을 차례로 점검하면 좋습니다.";
  }
  ...
  async function runCode() {
    ...
    if (!code) {
      ...
      setExplanation("입력 안내", "먼저 변수 만들기, print() 출력하기, type() 확인하기 중 하나를 코드 입력창에 직접 작성해 보세요.", true);
      return;
    }
